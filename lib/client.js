...
module.exports = function Client (specs, options) {
  ...
  if (specs.operations) {
    for (const operationKey in specs.operations) {
      const operation = specs.operations[operationKey]
      ...
      client[operationKey] = function (data, callback) {
        ...
        // translate data
        if (operation.data) {
          for (const position in operation.data) {
            debug('translating data:', position)
            const parameters = {}

            for (const parameter of operation.data[position].parameters) {
              ...
              if (parameter.name && data && (data[parameter.name] || data[parameter.as])) {
                ...
                throw new Error(`Required parameter is missing:${parameter.name}`)
              }
            }
